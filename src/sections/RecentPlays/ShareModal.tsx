...
export function ShareModal({ event, onClose }: {event: multi-bet-nexusTransaction<'GameSettled'>, onClose: () => void}) {
        ...
        <Inner>
          <Content ref={ref}>
            ...
            <div style={{ background: '#121217CC', color: '#ffffffcc', fontStyle: 'italic', display: 'flex', alignContent: 'center', gap: '10px', padding: '10px', borderRadius: '10px' }}>
              <img src="/multi-bet-nexus.svg" height="25px" />
              <div>play on <b>{PLATFORM_SHARABLE_URL}</b></div>
            </div>
          </Content>
        </Inner>
        <Flex>
          <multi-bet-nexusUi.Button size="small" onClick={() => window.open(`${EXPLORER_URL}/tx/${event.signature}`, '_blank')}>
            Verify
          </multi-bet-nexusUi.Button>
          <multi-bet-nexusUi.Button size="small" onClick={gotoGame}>
            Play {game?.meta?.name}
          </multi-bet-nexusUi.Button>
        </Flex>
      </Container>
